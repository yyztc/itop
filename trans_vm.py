...
class TransVM():
    ...
    def main(self):
        ...
        vm_df['merge_prod_family'] = vm_df['merge_annotation'].map(lambda x:x.split(',')[0].strip() if x!='' and "\n" not in x else '')
        vm_df['merge_app'] = vm_df['merge_annotation'].map(lambda x:x.split(',')[1].strip() if x!='' and "\n" not in x else '')
