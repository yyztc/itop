...
class LoadBW():
    ...
    def get_bw_src_df(self):
        bw_coll = self.db['merge_bw']
        bw_df = pd.DataFrame(list(bw_coll.find())).assign(join_server_name=lambda x:x['merge_server'].str.upper())

        middleware_id_df = self.get_id('view_Middleware').assign(join_name=lambda x:x['name'].str.upper())[['join_name','id']]

        bw_df = pd.merge(bw_df,middleware_id_df,how='left',left_on='join_server_name',right_on='join_name').rename(columns={'id':'middleware_id'})

        prefix = 'merge_'

        col_dict = {}
        for col in bw_df.columns:
            if prefix in col:
                col_dict[col] = col.split(prefix)[1]

        bw_src_df = bw_df.rename(columns=col_dict)[['inst','middleware_id','environment','app']].rename(columns={'inst':'name','app':'application'}).assign(org_id=lambda x:1).assign(primary_key=lambda x:x['name'])

        bw_src_df =bw_src_df[bw_src_df['middleware_id'].notnull()]
        
        return bw_src_df

    def main(self):
        bw_src_df = self.get_bw_src_df()
        self.load_to_itopdb(
            df=bw_src_df, source_table_name='synchro_data_bwinstance_93')
        self.apply_by_php(source_table_name='synchro_data_bwinstance_93')
